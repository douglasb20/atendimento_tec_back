...
@Entity('atendimento_chat_messages')
export class AtendimentoChatMessages {
  @Column({ name: 'id', type: 'char', length: 36, primary: true })
  @Generated('uuid')
  id: string;

  @Column({ name: 'atendimento_chat_id', type: 'bigint' })
  atendimento_chat_id: number;
  ...
  @ManyToOne(() => AtendimentoChats, (atendimentoChat) => atendimentoChat.atendimentoChatMessages)
  @JoinColumn({ name: 'atendimento_chat_id' })
  atendimentoChats: AtendimentoChats;

  @ManyToOne(() => Channels, (channel) => channel.atendimentoChatMessages)
